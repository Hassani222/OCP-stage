...
Le pipeline RAG se déroule en deux phases distinctes, qui ne se rejoignent qu'à un seul
endroit : ChromaDB. L'indexation y écrit (haut vers bas sur la figure~\ref{fig:archi-rag}),
la question/réponse y lit (bas vers haut), filtrée par l'identifiant du propriétaire
(\texttt{owner\_id}) — c'est ce filtre qui garantit qu'aucun document d'un autre utilisateur
n'apparaît jamais dans une réponse.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/architecture-rag.png}
\caption{Pipeline RAG de Clarté : indexation d'un document (haut) et génération d'une
réponse (bas), reliées par ChromaDB avec isolation par utilisateur.}
\label{fig:archi-rag}
\end{figure}
